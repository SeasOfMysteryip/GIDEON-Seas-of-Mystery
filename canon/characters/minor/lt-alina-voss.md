---
id: character.lt_alina_voss
name: Lt. Alina Voss
type: character
status: canon_locked
source_module: "03_Characters v2"
source_section: "LT. ALINA VOSS"
primary_role: "Fracture Recon Officer"
season_scope:
  - s1
aliases:
  - The Cold Cartographer
related_characters:
  - character.jeb_blackwater
related_relics:
  - relic.wrist_compass
canon_tags:
  - arclight
  - tactical_observer
  - clinical_counter_myth
  - fracture_mapping
---

# Lt. Alina Voss

## Core Identity

Lt. Alina Voss is a fracture recon officer and spectral-zone mapping specialist for ArcLight Division. She brings operational, scientific, and battlefield language into spaces dominated by myth and relic intuition.

## Archetype And Vibe

- Cold cartographer.
- Calm eye in the storm.
- Tactical analyst who survives by refusing mystic drift.

## Emotional Engine

Voss is driven by operational clarity and the need to map hostile anomaly space without surrendering to it symbolically.

## Voice Law

Voss speaks in clipped, tactical, clinical language.

### Canon-Aligned Dialogue Traits

- Metrics and field assessment.
- No mystical interpretation.
- Calm under anomaly pressure.
- Recommendation and risk language.

### Locked Examples

- "Sector margin degraded. Pulse stacking non-linear."
- "Skyfield drift indicates secondary breach proximity."
- "Recommend reducing ley exposure."

### Forbidden Voice Drift

- No mystical language.
- No emotional reflection speeches.
- No speculative mythology.

## Visual And Field Notes

- Athletic build, minimalist survival gear, cropped practical hair.
- Neutral-toned uniform built for field durability rather than display.
- Wrist compass or fracture-synced instrument should twitch, stutter, or misread before Voss does.
- Her gaze should be analytical rather than awed; mythic spectacle becomes data under her pressure.

## Operational Use

- Voss maps active ley drift, pulse compression, and hostile anomaly space.
- She works best as the counter-myth point of view: not cynical, not dismissive, but clinically unwilling to use mystical language.
- Her Season Two return should carry the EP104 Gus promise as human leverage under the tactical surface.

## Relationships

### Jeb Blackwater

Voss shows functional respect for Jeb's competence while mistrusting unregulated relic energy and mythic command styles.

### Gus Maddox

Has told Gus that if he ever needs help, to give her a call. (EP104.) She has Navy ties and Gus is a former Navy Seal.

## Canon Failure Test

If a scene with Voss does not reflect clinical fracture analysis, operational speech, or anti-myth discipline, Gideon should flag it.

