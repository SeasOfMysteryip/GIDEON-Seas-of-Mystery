---
id: episodes.s01.e101.index
type: episode_index
status: working
source_module: "Episode Teleplay / Drive scene packets / 00_Canon Bible Core v2"
source_section: "Episode 101 living bible insert + whole-episode source map"
source_file: "source-docs/00_Canon Bible Core🗺️ v2.docx"
uses:
  - episodes/s01/e101/source-map.md
  - episodes/s01/e101/script.md
  - episodes/s01/e101/continuity-notes.md
  - episodes/s01/e101/revision-notes.md
  - episodes/s01/e101/scene-cards/index.md
---

# Episode 101 Index

This folder stores the repo-native episode packet for `EP101 — The Girl from the Fog`.

Current contents:

- `source-map.md` for whole-episode and by-scene source tracking
- `script.md` for the clean episode script landing zone
- `continuity-notes.md` for episode-level canon and structure locks
- `revision-notes.md` for Forge / final-pass workflow notes
- `scene-cards/` for repo-native scene packets derived from the episode structure

## Authority Rule

Within this episode folder, use authority in this order:

1. `script.md`
2. `continuity-notes.md`
3. `scene-cards/`
4. `revision-notes.md`

If any episode file conflicts with locked canon in `canon/`, the locked canon files win.
